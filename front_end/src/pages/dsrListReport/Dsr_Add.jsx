...
//   try {
//     if (isEdit) {
//       await axios.put(`${API_URL}/dsr/update/${id}`, formData, {
//         headers: {
...
//       });
//     } else {
//       await axios.post(`${API_URL}/dsr/create`, formData, {
//         headers: {
//           Authorization: `Bearer ${token}`,
//           "Content-Type": "multipart/form-data",
//         },
...
export default function AddDSRForm() {
  ...
  useEffect(() => {
    ...
    axios
      ...
      .then((res) => {
        ...
        setProjects(existing.projects || [{ projectName: "", projectDescription: "", todoTask: "" }]);
      })
      .catch(() => toast.error("Failed to fetch DSR"));
  }, [id, isEdit]);
  ...
  const handleAddProject = () => {
    setProjects([...projects, { projectName: "", projectDescription: "", todoTask: "" }]);
  };
  ...
  return (
    <div className="min-h-screen text-white bg-black px-7 rounded-md py-10">
      ...
      >
        {/* Manager */}
        <div>
          <label className="block mb-1 text-sm font-semibold">Manager Name</label>
          <select
            className="w-full h-12 px-4 py-2 rounded-md border-white bg-neutral-800 text-white border"
            value={form.email}
            onChange={(e) => setForm({ ...form, email: e.target.value })}
